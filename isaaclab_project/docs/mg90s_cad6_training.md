...
## 현재 기본 Velocity v3 (2026-09-23)

`run_mg90s.ps1`의 기본 작업은 `Isaac-DKSH-MG90S-CAD6-Velocity-Direct-v3`이다.
v2와 18행동·69관측이 같아도 보행/명령 전환 계약이 다르므로 전용 새
체크포인트를 학습해야 한다. 새 실험 폴더는
`dksh_mg90s_cad6_velocity_l1v4`이다. 기존 v0–v2 등록과 보행 계산은
해당 체크포인트 재현을 위해 유지한다.

- +yaw는 URDF의 +Z hip 축과 몸체 좌표계에 맞춘다. 회전 발 접선속도는
  hip 장착 반경 0.120 m와 hip 기준 stance 발 반경을 합한 몸체 기준 반경으로 계산한다.
- 학습 명령 후보는 v2와 같이 전진 -0.025~0.050 m/s, 횡이동 ±0.025 m/s,
  회전 ±0.08 rad/s이다. 복합 명령은 발 이동량에 맞춰 투영한다.
- 에피소드 안에서도 4초마다 명령을 다시 뽑는다. 정지 명령도 전환 대상이 된다.
  도착 시간 관측과 보상은 각 명령 구간의 시작 자세를 기준으로 다시 계산한다.
- `evaluate_mg90s_cad6.py`는 평가 시 실제 표본의 정지 정책 오차를 함께
  측정하고, 평면·회전 양쪽 모두 정지 기준보다 20% 이상 작아야 통과시킨다.
  절대 오차 한계와 낙상·이탈·토크 조건도 유지한다. v3은 평가 중 명령 전환이
  실제로 관측되어야 한다. 리셋 및 명령 전환 프레임은 속도 오차 평균에서 제외한다.
- v3 학습 전에 원본 FBX의 manifest 해시와 USD/visuals/URDF 파일 존재를
  확인하고, 학습 파일 해시를 run metadata에 기록한다. v3 체크포인트 재개·평가 시
  현재 파일과 대조한다. 이는 자산의 동일성 검사이며, manifest가 명시한
  임시 축·질량·충돌체가 실물과 맞는다는 승인 판정은 아니다.
- `Episode_Count/*`는 리셋이 없는 스텝에는 0, 리셋 스텝에는 실제 종료 수를
  기록한다. RSL-RL 그래프의 값은 rollout 안의 스텝 평균이므로 총 에피소드 수는
  `run_metadata.json`의 `completed_episodes`를 사용한다.

`ros2/config/nav2_low_resource.yaml`의 속도 상한은 투영 없이 사용할 수 있는
v3의 보수적 부분 범위이다. Nav2가 생성한 명령이 실제 정책에 연결됐거나
실물에서 추종 가능하다는 뜻은 아니다. 현재 ROS2 `/cmd_vel` 경로는 Unity
어댑터까지 확인되며, `RSPI/scripts/CORE.py`는 관측과 ONNX 추론값만 출력한다.
Nav2의 진행 검사는 15초 동안 0.10 m 이동 또는 0.10 rad 회전을 인정하도록
조정했다. 실제 이동·진행 판정은 별도 검증이 필요하다.

기존 v0의 2,000 iteration 완료·평가 기록은 당시 평가 규칙으로 얻은 결과다.
v0 명령 샘플러의 seed 42 표본 100,000개에서 정지 정책 오차는 평면
0.00535190 m/s, 회전 0.03424469 rad/s로 기존 절대 오차 합격선 안이다.
보관 평가의 표본·시드는 다르므로 그 정책의 실제 정지 기준 대비 개선 여부는
재평가해야 한다. 당시 기록은 v3의 회전 기구학·명령 전환 성능을 증명하지 않는다.
2026-09-23 검토 PC에는 Isaac Sim/PyTorch 실행 환경이 확인되지 않아 v3 물리 사전 검사,
재학습, 체크포인트 평가는 아직 수행되지 않았다.
...
### 속도 추종 재학습 v2: 도착 시각 shaping (2026-09-22)

`Isaac-DKSH-MG90S-CAD6-Velocity-Direct-v2`는 이전 설정이며,
현재 기본 실행은 위 v3이다. 이 절의 `run_mg90s.ps1` 예시 명령도 현재는
v3을 실행하며, v2 재현에는 스크립트에 v2 작업 ID를 명시해야 한다.
새 학습 폴더는 `dksh_mg90s_cad6_velocity_l1v3`이며, 기존 v0/v1 체크포인트는
작업 ID와 69개 관측 검증에서 거절된다. 기존 등록과 보관 정책은 이전 결과 재현용이다.
...
- 정지 후 남은 19초를 도착 창으로 사용한다. 정책 관측에는 반복하지 않는
  `arrival_phase` 한 값을 추가해 69개가 된다. 이 값은 현재 시점의 목표 궤적
  비율이며, 같은 자세라도 너무 이른지 늦었는지를 정책이 구분하게 한다.
- 명령 twist를 에피소드 시작 좌표계의 기준 궤적으로 적분한다. 접선 방향에서
  앞서 간 경우(overshoot)와 뒤처진 경우(undershoot)를 각각 목표 이동량으로
  정규화해 벌점으로 적용하고, 곡선 궤적의 횡방향 이탈과 yaw 시간 오차도
  함께 벌점으로 둔다. 10mm 미만 병진 또는 0.02rad 미만 yaw 목표, 즉 정지에
  가까운 명령에는 이 벌점이 적용되지 않는다.
- 이 shaping은 목표 도달 시점을 맞추는 항목이며 물리 한계를 늘리지 않는다.
  실제 도달 가능한 명령 범위와 속도 정확도는 별도 평가로 검증해야 한다.
...
평가에는 `evaluate_mg90s_cad6.py --task Isaac-DKSH-MG90S-CAD6-Velocity-Direct-v2`을
...
루트 실행기의 기본 프로필은 `velocity`이며 현재 v3을 선택한다. 이전 직진 또는 sprint 정책을 다룰 때만
